...
namespace ConsoleApp1
{
    public class Program
    {
        ...
        private static void Timer_Tick(object sender, EventArgs e)
        {
            ...
            LastInput.dwTime = 0;
            if (!Interop.GetLastInputInfo(ref LastInput)) return;
            int IdleTime = Environment.TickCount - LastInput.dwTime;

            Console.WriteLine($"No user input: {IdleTime}ms");

            if (IdleTime < 5000) return; //5 seconds
            DoSoftwareProcess();

        }

        public static void DoSoftwareProcess()
        {
            ResetUserIdle idle = new ResetUserIdle();

            Console.WriteLine("Software doing some heavy process, 5 seconds");
    ...
    public class ResetUserIdle : IDisposable
    {
        private Thread Thread { get; set; }

        public ResetUserIdle()
        {
            Thread = new Thread(IdleTimeReseter.Start);
            Thread.Start();
        }
        ~ResetUserIdle()
        {
    ...
    public static class IdleTimeReseter
    {
        ...
        public static void Start()
        {
            ...
            while (true)
            {
                ...
                if (IdleTime < 2000) return; //2 seconds
                ResetIdle();
                Thread.Sleep(100);
            }
        }
        ...
        private static void SendKeyPress(KeyCode keyCode)
        {
            ...
            if (Interop.SendInput((uint)inputs.Length, inputs, Marshal.SizeOf(typeof(INPUT))) == 0) throw new Exception();
        }
